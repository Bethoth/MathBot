...
// *************************************************** help embeds ***************************************************
    ...
    calcHelp.addField("Comment on s'en sert ?", `Cette commande s'utilise ainsi : \`math calc [nombre] [opérateur] [nombre]\``);
    calcHelp.addField("Exemples:", `
    \`math calc 7 + 7\` → addition → 14
    \`math calc 7 - 5\` → soustraction → 2
    \`math calc 7 * 5\` → multiplication → 35
    \`math calc 10 / 5\` → division → 2
    `);
    ...
    perimeterHelp.addField("À quoi ça sert ?", "Cette commande sert à calculer le périmètre de nombreuses figures (\`math pl\`)");
    perimeterHelp.addField("Comment on s'en sert ? (soit n le nombre de mesures)", `
    Cette commande s'utilise ainsi : \`math perimeter [figure] n[mesure]\`
    `);
    perimeterHelp.addField("Exemples:", `
    \`math perimeter square 5\` → périmètre d'un carré → 20
    \`math perimeter rectangle 8 6\` → périmètre d'un rectangle → 28
    `);
    ...
    areaHelp.addField("À quoi ça sert ?", "Cette commande sert à calculer l'aire de nombreuses figures (\`math al\`)");
    areaHelp.addField("Comment on s'en sert ? (soit n le nombre de mesures)", `Cette commande s'utilise ainsi : \`math area [figure] n[mesure]\``);
    areaHelp.addField("Exemples:", `
    \`math area square 5\` → aire d'un carré → 25
    \`math area rectangle 8 6\` → aire d'un rectangle → 48
    `);
    ...
    volumeHelp.addField("À quoi ça sert ?", "Cette commande sert à calculer le volume de nombreuses figures (\`math vl\`)");
    volumeHelp.addField("Comment on s'en sert ? (soit n le nombre de mesures)", `Cette commande s'utilise ainsi : \`math volume [figure] n[mesure]\``);
    volumeHelp.addField("Exemples:", `
    \`math volume cube 5\` → volume d'un carré → 125
    \`math volume rectangleCuboid 8 6 5\` → volume d'un rectangle → 240
    `);
    ...
    theoremHelp.addField("À quoi ça sert ?", "Cette commande sert à effectuer quelques théorèmes (\`math tl\`)");
    theoremHelp.addField("Comment on s'en sert ? (soit n le nombre de mesures)", `Cette commande s'utilise ainsi : \`math theorem [théorème] n[mesure]\``);
    theoremHelp.addField("Exemples:", `
    \`math theorem pythagoreHypotenuse 5 5\` → calcul d'hypoténuse → 7 (environ)
    \`math theorem pythagoreOtherSide 8 6\` → calcul d'un autre côté → 5 (environ)
    `);
